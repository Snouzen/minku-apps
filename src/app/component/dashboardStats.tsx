...
import { CheckCircle2, AlertCircle, User2 } from "lucide-react";
...
export default function DashboardStats() {
  ...
  const pics =
    currentUser?.role === "super_admin"
      ? ["Agung", "Latifah", "Pepy", "Pandu", "Fifi", "Rama"]
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        {pics.map((name) => {
          ...
          return (
            <div
              key={name}
              className={`bg-white p-6 rounded-[2rem] border transition-all duration-300 relative overflow-hidden shadow-sm
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  {hasWarning && (
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="px-2 py-0.5 rounded-full text-[9px] font-black bg-red-100 text-red-600">
                        {almostExpiredCount}
                      </span>
                    </div>
                  )}
  ...
  const getStatusCard = (
    ...
    <div
      className={`bg-white p-6 rounded-[2rem] border transition-all duration-300 relative overflow-hidden shadow-sm ${bgColor} border-gray-100 hover:border-orange-200`}
    >
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
